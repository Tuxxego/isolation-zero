...
//
// Purpose: The downtrodden fatcits of City 17. Timid when unarmed, they will
...
//-----------------------------------------------------------------------------
// FatCitizen expression types
//-----------------------------------------------------------------------------
enum FatCitizenExpressionTypes_t
{
...
class CNPC_FatCitizen : public CNPC_PlayerCompanion
{
	DECLARE_CLASS( CNPC_FatCitizen, CNPC_PlayerCompanion );
public:
	CNPC_FatCitizen()
	 :	m_iHead( -1 )
#ifdef EZ
	...
	void			Precache();
	void			PrecacheAllOfType( FatCitizenType_t );
	void			Spawn();
	void			PostNPCInit();
...
#ifdef EZ
	WeaponProficiency_t CalcWeaponProficiency(CBaseCombatWeapon *pWeapon); // Added by 1upD - FatCitizen proficiency should be configurable
...
private:
	...
	enum
	{
		COND_FAT_PLAYERHEALREQUEST = BaseClass::NEXT_CONDITION,
		COND_FAT_COMMANDHEAL,
		COND_FAT_HURTBYFIRE,
		COND_FAT_START_INSPECTION,
#ifdef EZ
		COND_FAT_WILLPOWER_LOW,
		COND_FAT_WILLPOWER_HIGH,
		NEXT_CONDITION,
#endif
		...
		TASK_FAT_HEAL = BaseClass::NEXT_TASK,
		TASK_FAT_RPG_AUGER,
		TASK_FAT_PLAY_INSPECT_SEQUENCE,
		TASK_FAT_SIT_ON_TRAIN,
		TASK_FAT_LEAVE_TRAIN,
		TASK_FAT_SPEAK_MOURNING,
#ifdef HL2_EPISODIC
		TASK_FAT_HEAL_TOSS,
#endif
	...
	FatCitizenType_t	m_Type;
	FatCitizenExpressionTypes_t	m_ExpressionType;
...
//---------------------------------------------------------
//---------------------------------------------------------
inline bool CNPC_FatCitizen::NearCommandGoal()
{
...
//---------------------------------------------------------
//---------------------------------------------------------
inline bool CNPC_FatCitizen::VeryFarFromCommandGoal()
{
...
//
//==============================================================================
#define CITIZEN_RESPONSE_DISTANCE			768			// Maximum distance for responding fatcits
...
class CFatCitizenResponseSystem : public CBaseEntity
{
	DECLARE_CLASS( CFatCitizenResponseSystem, CBaseEntity );
public:
	DECLARE_DATADESC();
	...
	void	AddResponseTrigger( fatcitresponses_t	iTrigger );
...
class CSquadFatInsignia : public CBaseAnimating
{
	DECLARE_CLASS( CSquadFatInsignia, CBaseAnimating );
	void Spawn();
};
