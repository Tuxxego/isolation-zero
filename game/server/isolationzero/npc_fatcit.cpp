...
//
// Purpose: The downtrodden fatcits of City 17.
//
//=============================================================================//

#include "cbase.h"

#include "npc_fatcit.h"
...
ConVar	sk_fatcit_health				( "sk_fatcit_health",					"0");
ConVar	sk_fatcit_heal_player			( "sk_fatcit_heal_player",				"25");
ConVar	sk_fatcit_heal_player_delay	( "sk_fatcit_heal_player_delay",		"25");
ConVar	sk_fatcit_giveammo_player_delay( "sk_fatcit_giveammo_player_delay",	"10");
ConVar	sk_fatcit_heal_player_min_pct	( "sk_fatcit_heal_player_min_pct",		"0.60");
ConVar	sk_fatcit_heal_player_min_forced( "sk_fatcit_heal_player_min_forced",		"10.0");
ConVar	sk_fatcit_heal_ally			( "sk_fatcit_heal_ally",				"30");
ConVar	sk_fatcit_heal_ally_delay		( "sk_fatcit_heal_ally_delay",			"20");
ConVar	sk_fatcit_heal_ally_min_pct	( "sk_fatcit_heal_ally_min_pct",		"0.90");
ConVar	sk_fatcit_player_stare_time	( "sk_fatcit_player_stare_time",		"1.0" );
ConVar  sk_fatcit_player_stare_dist	( "sk_fatcit_player_stare_dist",		"72" );
ConVar	sk_fatcit_stare_heal_time		( "sk_fatcit_stare_heal_time",			"5" );
#ifdef EZ
ConVar  sk_fatcit_ar2_proficiency("sk_fatcit_ar2_proficiency", "2"); // Added by 1upD. Skill rating 0 - 4 of how accurate the AR2 should be
ConVar  sk_fatcit_default_proficiency("sk_fatcit_default_proficiency", "1"); // Added by 1upD. Skill rating 0 - 4 of how accurate all weapons but the AR2 should be
ConVar  sk_fatcit_default_willpower("sk_fatcit_default_willpower", "1"); // Added by 1upD. Amount of mental stress damage fatcit can take before panic
#endif

ConVar	g_ai_fatcit_show_enemy( "g_ai_fatcit_show_enemy", "0" );

ConVar	npc_fatcit_insignia( "npc_fatcit_insignia", "0" );
ConVar	npc_fatcit_squad_marker( "npc_fatcit_squad_marker", "0" );
ConVar	npc_fatcit_explosive_resist( "npc_fatcit_explosive_resist", "0" );
ConVar	npc_fatcit_auto_stupidplayer_squad( "npc_fatcit_auto_stupidplayer_squad", "1" );
ConVar	npc_fatcit_auto_stupidplayer_squad_allow_use( "npc_fatcit_auto_stupidplayer_squad_allow_use", "0" );

ConVar	npc_fatcit_dont_precache_all( "npc_fatcit_dont_precache_all", "0" );


ConVar  npc_fatcit_medic_emit_sound("npc_fatcit_medic_emit_sound", "1" );
#ifdef HL2_EPISODIC
// todo: bake these into pound constants (for now they're not just for tuning purposes)
ConVar  npc_fatcit_heal_chuck_medkit("npc_fatcit_heal_chuck_medkit" , "1" , FCVAR_ARCHIVE, "Set to 1 to use new experimental healthkit-throwing medic.");
ConVar npc_fatcit_medic_throw_style( "npc_fatcit_medic_throw_style", "1", FCVAR_ARCHIVE, "Set to 0 for a lobbier trajectory" );
ConVar npc_fatcit_medic_throw_speed( "npc_fatcit_medic_throw_speed", "650" );
ConVar	sk_fatcit_heal_toss_player_delay("sk_fatcit_heal_toss_player_delay", "26", FCVAR_NONE, "how long between throwing healthkits" );


#define MEDIC_THROW_SPEED npc_fatcit_medic_throw_speed.GetFloat()
#define USE_EXPERIMENTAL_MEDIC_CODE() (npc_fatcit_heal_chuck_medkit.GetBool() && NameMatches("griggs"))
#endif

ConVar stupidplayer_squad_autosummon_time( "stupidplayer_squad_autosummon_time", "5" );
ConVar stupidplayer_squad_autosummon_move_tolerance( "stupidplayer_squad_autosummon_move_tolerance", "20" );
ConVar stupidplayer_squad_autosummon_player_tolerance( "stupidplayer_squad_autosummon_player_tolerance", "10" );
ConVar stupidplayer_squad_autosummon_time_after_combat( "stupidplayer_squad_autosummon_time_after_combat", "8" );
ConVar stupidplayer_squad_autosummon_debug( "stupidplayer_squad_autosummon_debug", "0" );
#ifdef EZ
ConVar fatai_debug_willpower("fatai_debug_willpower", "0"); // 1upD - use this to print messages to the log about FatCitizen willpower
ConVar fatai_debug_rebel_suppressing_fire("fatai_debug_rebel_suppressing_fire", "0"); // 1upD - use this to print messages to the log about FatCitizen willpower
ConVar fatai_min_suppression_distance("fatai_min_suppression_distance", "256", FCVAR_REPLICATED); // 1upD - minimum distance from object for an NPC to use suppressing fire
ConVar fatai_suppression_distance_ratio("fatai_suppression_distance_ratio", "0.5", FCVAR_REPLICATED); // 1upD - What percent of distance to suppression target must be covered
ConVar fatai_willpower_translate_schedules("fatai_willpower_translate_schedules", "1", FCVAR_REPLICATED); // 1upD - Should new EZ2 schedules be used?
#endif
#define ShouldAutosquad() (npc_fatcit_auto_stupidplayer_squad.GetBool())
...
// Animation events
int AE_FATIZEN_GET_PACKAGE;
int AE_FATIZEN_HEAL;

//-------------------------------------
//-------------------------------------

ConVar	fatai_follow_move_commands( "fatai_follow_move_commands", "1" );
ConVar	ai_fatcit_debug_commander( "ai_fatcit_debug_commander", "1" );
#define DebuggingCommanderMode() (ai_fatcit_debug_commander.GetBool() && (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))

//-----------------------------------------------------------------------------
// FatCitizen expressions for the fatcit expression types
//-----------------------------------------------------------------------------
#define STATES_WITH_EXPRESSIONS		3		// Idle, Alert, Combat
#define EXPRESSIONS_PER_STATE		1

char *szExpressionTypesFat[CIT_EXP_LAST_TYPE] =
{
	"Unassigned",
	"Scared",
	"Normal",
...
struct fatcit_expression_list_t
{
	char *szExpressions[EXPRESSIONS_PER_STATE];
};
// Scared
fatcit_expression_list_t ScaredExpressions[STATES_WITH_EXPRESSIONS] =
{
	{ "scenes/Expressions/fatcit_scared_idle_01.vcd" },
	{ "scenes/Expressions/fatcit_scared_alert_01.vcd" },
	{ "scenes/Expressions/fatcit_scared_combat_01.vcd" },
};
// Normal
fatcit_expression_list_t NormalExpressions[STATES_WITH_EXPRESSIONS] =
{
	{ "scenes/Expressions/fatcit_normal_idle_01.vcd" },
	{ "scenes/Expressions/fatcit_normal_alert_01.vcd" },
	{ "scenes/Expressions/fatcit_normal_combat_01.vcd" },
};
// Angry
fatcit_expression_list_t AngryExpressions[STATES_WITH_EXPRESSIONS] =
{
	{ "scenes/Expressions/fatcit_angry_idle_01.vcd" },
	{ "scenes/Expressions/fatcit_angry_alert_01.vcd" },
	{ "scenes/Expressions/fatcit_angry_combat_01.vcd" },
};
...
class CCommandPointFat : public CPointEntity
{
	DECLARE_CLASS( CCommandPointFat, CPointEntity );
public:
	CCommandPointFat()
		: m_bNotInTransition(false)
	{
		if ( ++gm_nCommandPoints > 1 )
			DevMsg( "WARNING: More than one fatcit command point present\n" );
	}
...
int CCommandPointFat::gm_nCommandPoints;

LINK_ENTITY_TO_CLASS( info_target_command_point, CCommandPointFat );
BEGIN_DATADESC( CCommandPointFat )
...
//---------------------------------------------------------
// FatCitizen activities
//---------------------------------------------------------

int ACT_FAT_HANDSUP;
int	ACT_FAT_BLINDED;		// Blinded by scanner photo
int ACT_FAT_SHOWARMBAND;
int ACT_FAT_HEAL;
int	ACT_FAT_STARTLED;		// Startled by sneaky scanner

//---------------------------------------------------------

LINK_ENTITY_TO_CLASS( npc_fatcit, CNPC_FatCitizen );

//---------------------------------------------------------

BEGIN_DATADESC( CNPC_FatCitizen )
...
//						m_FuncTankBehavior
	...
	DEFINE_KEYFIELD(	m_Type, 					FIELD_INTEGER,	"fatcittype" ),
...
CSimpleSimTimer CNPC_FatCitizen::gm_PlayerSquadEvaluateTimer;

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------

bool CNPC_FatCitizen::CreateBehaviors()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::Precache()
{
	SelectModel();
	SelectExpressionType();

	if ( !npc_fatcit_dont_precache_all.GetBool() )
	...
	PrecacheScriptSound( "NPC_FatCitizen.FootstepLeft" );
	PrecacheScriptSound( "NPC_FatCitizen.FootstepRight" );
	PrecacheScriptSound( "NPC_FatCitizen.Die" );

	PrecacheInstancedScene( "scenes/Expressions/FatCitizenIdle.vcd" );
	PrecacheInstancedScene( "scenes/Expressions/FatCitizenAlert_loop.vcd" );
	PrecacheInstancedScene( "scenes/Expressions/FatCitizenCombat_loop.vcd" );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::PrecacheAllOfType( FatCitizenType_t type )
{
	if ( m_Type == CT_UNIQUE )
		return;
...
void CNPC_FatCitizen::Spawn()
{
	...
	m_iHealth = sk_fatcit_health.GetFloat();
	...
	m_iszIdleExpression = MAKE_STRING("scenes/expressions/fatcitidle.vcd");
	m_iszAlertExpression = MAKE_STRING("scenes/expressions/fatcitalert_loop.vcd");
	m_iszCombatExpression = MAKE_STRING("scenes/expressions/fatcitcombat_loop.vcd");
	...
	SetUse( &CNPC_FatCitizen::CommanderUse );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::PostNPCInit()
{
	...
	if ( IsInPlayerSquad() )
	{
		if ( m_pSquad->NumMembers() > MAX_PLAYER_SQUAD )
			DevMsg( "Error: Spawning fatcit in player squad but exceeds squad limit of %d members\n", MAX_PLAYER_SQUAD );
...
void CNPC_FatCitizen::SelectModel()
{
	...
	{
		struct FatCitizenTypeMapping
		{
			const char *pszMapTag;
			FatCitizenType_t type;
		};

		static FatCitizenTypeMapping FatCitizenTypeMappings[] = 
		{
		...
		for ( int i = 0; i < ARRAYSIZE(FatCitizenTypeMappings); i++ )
		{
			if ( Q_stristr( szMapName, FatCitizenTypeMappings[i].pszMapTag ) )
			{
				m_Type = FatCitizenTypeMappings[i].type;
				break;
			}
		}
	...
	if( HasSpawnFlags( SF_CITIZEN_RANDOM_HEAD | SF_CITIZEN_RANDOM_HEAD_MALE | SF_CITIZEN_RANDOM_HEAD_FEMALE ) || GetModelName() == NULL_STRING )
	{
		...
		else
		{
			...
			{
				CNPC_FatCitizen *pFatCitizen = dynamic_cast<CNPC_FatCitizen *>(g_AI_Manager.AccessAIs()[i]);
				if ( pFatCitizen && pFatCitizen != this && pFatCitizen->m_iHead >= 0 && pFatCitizen->m_iHead < ARRAYSIZE(g_ppszRandomHeads) )
				{
					headCounts[pFatCitizen->m_iHead]++;
				}
			}
	...
	// Unique fatcit models are left alone
	if ( m_Type != CT_UNIQUE )
	{
#ifdef EZ2
		...
		// If this fatcit's type is "LongFall", they should use the appropriate subtype
		if (m_Type == CT_LONGFALL)
			subtype = "x"; // We may wish to change this so that long fall boot rebels are Group04. We would do this in order to get a separate -b and -m group for long fall fatcits - for now let's not bother
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::SelectExpressionType()
{
	...
	{
	case CT_DOWNTRODDEN:
		m_ExpressionType = (FatCitizenExpressionTypes_t)RandomInt( CIT_EXP_SCARED, CIT_EXP_NORMAL );
		break;
	case CT_REFUGEE:
		m_ExpressionType = (FatCitizenExpressionTypes_t)RandomInt( CIT_EXP_SCARED, CIT_EXP_NORMAL );
		break;
	case CT_REBEL:
		m_ExpressionType = (FatCitizenExpressionTypes_t)RandomInt( CIT_EXP_SCARED, CIT_EXP_ANGRY );
		break;
	case CT_BRUTE:
		m_ExpressionType = (FatCitizenExpressionTypes_t)RandomInt(CIT_EXP_NORMAL, CIT_EXP_ANGRY); // Brutes show no fear
	case CT_LONGFALL:
		m_ExpressionType = (FatCitizenExpressionTypes_t)RandomInt(CIT_EXP_NORMAL, CIT_EXP_ANGRY); // Long fall boot rebels are crazy
	case CT_DEFAULT:
	case CT_UNIQUE:
	default:
		break;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::FixupMattWeapon()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::OnRestore()
{
	gm_PlayerSquadEvaluateTimer.Force();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
string_t CNPC_FatCitizen::GetModelName() const
{
	...
	if (!Q_strnicmp(STRING(iszModelName), "models/c17_", 11) ||
		...
		!Q_strnicmp(STRING(iszModelName), "models/fatcit", 14))
	{
		return NULL_STRING;
	}
...
//-----------------------------------------------------------------------------
Class_T	CNPC_FatCitizen::Classify()
{
	if (GlobalEntity_GetState("gordon_precriminal") == GLOBAL_ON)
		return CLASS_CITIZEN_PASSIVE;

	if (GlobalEntity_GetState("fatcits_passive") == GLOBAL_ON)
		return CLASS_CITIZEN_PASSIVE;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::ShouldAlwaysThink() 
{ 
...
bool CNPC_FatCitizen::ShouldBehaviorSelectSchedule( CAI_BehaviorBase *pBehavior )
{
...
void CNPC_FatCitizen::OnChangeRunningBehavior( CAI_BehaviorBase *pOldBehavior,  CAI_BehaviorBase *pNewBehavior )
{
...
//-----------------------------------------------------------------------------
#ifdef EZ
void CNPC_FatCitizen::GatherWillpowerConditions()
{
	if (m_bWillpowerDisabled || m_NPCState != NPC_STATE_COMBAT)
		return;

	CBaseCombatWeapon * pWeapon = GetActiveWeapon();
	if (pWeapon == NULL) // Unarmed fatcits do not use willpower
		return;

	int l_iWillpower = sk_fatcit_default_willpower.GetInt() + m_iWillpowerModifier;
	...
	// Bullet impacts make fatcits jumpy
	...
	if (l_iWillpower <= 0)
	{
		if (!HasCondition(COND_FAT_WILLPOWER_LOW))
		{
		...
		ClearCondition(COND_FAT_WILLPOWER_HIGH);
		SetCondition(COND_FAT_WILLPOWER_LOW);
	}
	else if (l_iWillpower > 1)
	{
		if (!HasCondition(COND_FAT_WILLPOWER_HIGH))
			MsgWillpower("%s is now brave!\tWillpower: %i\n", l_iWillpower);

		ClearCondition(COND_FAT_WILLPOWER_LOW);
		SetCondition(COND_FAT_WILLPOWER_HIGH);
	}
	else {
		ClearCondition(COND_FAT_WILLPOWER_HIGH);
		ClearCondition(COND_FAT_WILLPOWER_LOW);
...
Disposition_t CNPC_FatCitizen::IRelationType(CBaseEntity * pTarget)
{
	Disposition_t disposition = BaseClass::IRelationType(pTarget);
	// If the fatcit has low willpower, fear an enemy instead of hating them
	if (disposition == D_HT && HasCondition(COND_FAT_WILLPOWER_LOW)) {
		return D_FR;
	}
	return disposition;
}

void CNPC_FatCitizen::MsgWillpower(const tchar * pMsg, int willpower)
{
	if (fatai_debug_willpower.GetBool() && m_flLastWillpowerMsgTime <= gpGlobals->curtime) {
...
//
//-----------------------------------------------------------------------------
const char* CNPC_FatCitizen::GetSquadSlotDebugName(int iSquadSlot)
{
	switch (iSquadSlot)
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::GatherConditions()
{
	...
	if ( !SpokeConcept( TLK_JOINPLAYER ) && IsRunningScriptedSceneWithSpeech( this, true ) )
	{
		...
		for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
		{
			...
			{
				(assert_cast<CNPC_FatCitizen *>(pNpc))->SetSpokeConcept( TLK_JOINPLAYER, NULL );
			}
		}
	}
	...
	if( CanHeal() )
	{
		...
		float flStareDist = sk_fatcit_player_stare_dist.GetFloat();
		...
		if( pPlayer->IsAlive() && flPlayerDamage > 0 && (flDistSqr <= flStareDist * flStareDist) && pPlayer->FInViewCone( this ) && pPlayer->FVisible( this ) )
		{
			...
			if( gpGlobals->curtime - m_flTimePlayerStare >= sk_fatcit_player_stare_time.GetFloat() && gpGlobals->curtime > m_flTimeNextHealStare && !IsCurSchedule( SCHED_CITIZEN_HEAL ) )
			{
				if ( ShouldHealTarget( pPlayer, true ) )
				{
					SetCondition( COND_FAT_PLAYERHEALREQUEST );
				}
				else
				{
					m_flTimeNextHealStare = gpGlobals->curtime + sk_fatcit_stare_heal_time.GetFloat() * .5f;
					ClearCondition( COND_FAT_PLAYERHEALREQUEST );
				}
			}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::PredictPlayerPush()
{
	if ( !AI_IsSinglePlayer() )
		return;

	if ( HasCondition( COND_FAT_PLAYERHEALREQUEST ) )
		return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::PrescheduleThink()
{
	...
	if ( !npc_fatcit_insignia.GetBool() && npc_fatcit_squad_marker.GetBool() && IsInPlayerSquad() )
	{
	...
	if( GetEnemy() && g_ai_fatcit_show_enemy.GetBool() )
	{
...
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::BuildScheduleTestBits()
{
	BaseClass::BuildScheduleTestBits();

	if ( IsCurSchedule( SCHED_IDLE_STAND ) || IsCurSchedule( SCHED_ALERT_STAND ) )
	{
		SetCustomInterruptCondition( COND_FAT_START_INSPECTION );
	}
	...
	if ( IsMedic() && IsCustomInterruptConditionSet( COND_HEAR_MOVE_AWAY ) )
	{
		if( !IsCurSchedule(SCHED_RELOAD, false) )
		{
			...
			SetCustomInterruptCondition( COND_FAT_PLAYERHEALREQUEST );
		}

		SetCustomInterruptCondition( COND_FAT_COMMANDHEAL );
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::FInViewCone( CBaseEntity *pEntity )
{
#if 0
	if ( IsMortar( pEntity ) )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	...
	case SCHED_NEW_WEAPON:
		...
		// has put this off for 10 seconds under the assumption that the fatcit would be able to 
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::SelectSchedule()
{
	// If we can't move, we're on a train, and should be sitting.
	if ( GetMoveType() == MOVETYPE_NONE )
	{
		// For now, we're only ever parented to trains. If you hit this assert, you've parented a fatcit
	...
	{
		DevMsg( "FatCitizen in select schedule but RPG is guiding?\n");
		pRPG->StopGuiding();
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::SelectSchedulePriorityAction()
{
...
//-----------------------------------------------------------------------------
// Determine if fatcit should perform heal action.
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::SelectScheduleHeal()
{
	...
	if ( CanHeal() )
	{
		...
		if ( pEntity )
		{
			if ( USE_EXPERIMENTAL_MEDIC_CODE() && IsMedic() )
			{
				...
				if ( ShouldHealTossTarget( pEntity, HasCondition( COND_FAT_PLAYERHEALREQUEST ) ) )
				{
					SetTarget( pEntity );
			...
			else if ( PlayerInRange( GetLocalOrigin(), HEAL_MOVE_RANGE ) )
			{
				...
				if ( ShouldHealTarget( pEntity, HasCondition( COND_FAT_PLAYERHEALREQUEST ) ) )
				{
					SetTarget( pEntity );
	...
	if ( CanHeal() )
	{
		...
		if ( pEntity && ShouldHealTarget( pEntity, HasCondition( COND_FAT_PLAYERHEALREQUEST ) ) )
		{
			SetTarget( pEntity );
			return SCHED_CITIZEN_HEAL;
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::SelectScheduleRetrieveItem()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::SelectScheduleNonCombat()
{
	if ( m_NPCState == NPC_STATE_IDLE )
	{
		// Handle being inspected by the scanner
		if ( HasCondition( COND_FAT_START_INSPECTION ) )
		{
			ClearCondition( COND_FAT_START_INSPECTION );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::SelectScheduleManhackCombat()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::SelectScheduleCombat()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::ShouldDeferToFollowBehavior()
{
#if 0
	if ( HaveCommandGoal() )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::TranslateSchedule( int scheduleType ) 
{
#ifdef EZ
	if (!m_bWillpowerDisabled && fatai_willpower_translate_schedules.GetBool()) {
		int willpowerSchedule = TranslateWillpowerSchedule(scheduleType);
		if (willpowerSchedule != SCHED_NONE) {
			if (fatai_debug_willpower.GetBool())
				DevMsg("FatCitizen name: %s\tOld Schedule: %s\t New Schedule: %s\tWillpower: %s%s\n", GetDebugName(), GetSchedule(scheduleType)->GetName(), GetSchedule(willpowerSchedule)->GetName(), HasCondition(COND_FAT_WILLPOWER_HIGH) ? "High" : "", HasCondition(COND_FAT_WILLPOWER_LOW) ? "Low" : "");
...
// (COND_FAT_WILLPOWER_LOW, COND_FAT_WILLPOWER_HIGH)
//		1upD
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::TranslateWillpowerSchedule(int scheduleType)
{
	switch (scheduleType)
	{
		// Don't investigate sounds if you're scared or unarmed
		case SCHED_INVESTIGATE_SOUND:
		if (HasCondition(COND_FAT_WILLPOWER_LOW) || HasCondition(COND_NO_WEAPON) || !OccupyStrategySlot(SQUAD_SLOT_CITIZEN_INVESTIGATE))
		{
			return SCHED_ALERT_FACE_BESTSOUND;
		}
		break;
	case SCHED_CHASE_ENEMY:
		if (HasCondition(COND_FAT_WILLPOWER_LOW) || HasCondition(COND_NO_WEAPON))
			return SCHED_RUN_FROM_ENEMY; // If we are afraid, take cover!
		break;
	case SCHED_COMBAT_FACE:
		// 1upD - don't just stand there, do something!
		if (HasCondition(COND_FAT_WILLPOWER_HIGH) && !HasCondition(COND_NO_WEAPON) && OccupyStrategySlot(SQUAD_SLOT_CITIZEN_ADVANCE)) // You have to be pretty brave to just rush at the enemy
	...
	case SCHED_RANGE_ATTACK1:
		...
		else if (HasCondition(COND_FAT_WILLPOWER_HIGH) && OccupyStrategySlot(SQUAD_SLOT_CITIZEN_ADVANCE) && !HasCondition(COND_NO_PRIMARY_AMMO))
		{
	...
	case SCHED_HIDE_AND_RELOAD:
		...
		if (pEnemy && fatai_debug_willpower.GetBool())
		...
		if (pEnemy
			&& !HasCondition(COND_FAT_WILLPOWER_LOW)
 ...
 //-----------------------------------------------------------------------------
 // Purpose: trace a firing position to see if this fatcit can suppress an enemy's position.
 //		1upD
 //-----------------------------------------------------------------------------
int CNPC_FatCitizen::TranslateSuppressingFireSchedule(int scheduleType)
{
	...
	if (!pEnemy) 
	{
		if(fatai_debug_rebel_suppressing_fire.GetBool())
			DevMsg("NPC_FatCitizen::TranslateSuppressingFireSchedule: %s tried to suppress but couldn't get enemy! \n", GetDebugName());
		return scheduleType;
	}
	...
	{
		if (fatai_debug_rebel_suppressing_fire.GetBool())
			DevMsg("NPC_FatCitizen::TranslateSuppressingFireSchedule: %s tried to suppress but hasn't shot at any enemies yet! \n", GetDebugName());
		return scheduleType;
	}
	...
	if (fatai_debug_rebel_suppressing_fire.GetBool() && distance <= fatai_min_suppression_distance.GetFloat())
	{
		DevMsg("NPC_FatCitizen::TranslateSuppressingFireSchedule: %s too close to target to use suppressing fire at range: %f\n", GetDebugName(), distance);
	}
	...
	if (fatai_debug_rebel_suppressing_fire.GetBool() && tr.fraction < fatai_suppression_distance_ratio.GetFloat())
	{
		DevMsg("NPC_FatCitizen::TranslateSuppressingFireSchedule: %s covering fire doesn't cover sufficent ratio at: %f\n", GetDebugName(), tr.fraction);
	}

	if (traceLength > fatai_min_suppression_distance.GetFloat() && tr.fraction >= fatai_suppression_distance_ratio.GetFloat())
	{
		if (fatai_debug_rebel_suppressing_fire.GetBool())
			DevMsg(UTIL_VarArgs("NPC_FatCitizen::TranslateSuppressingFireSchedule: %s using suppressing fire at range: %f\n", GetDebugName(), traceLength));
		
		return SCHED_CITIZEN_RANGE_ATTACK1_SUPPRESS;
	}
	else if (fatai_debug_rebel_suppressing_fire.GetBool()) {
		DevMsg(UTIL_VarArgs("NPC_FatCitizen::TranslateSuppressingFireSchedule: %s failed to use suppressing fire at range: %f\n", GetDebugName(), traceLength));
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::ShouldAcceptGoal( CAI_BehaviorBase *pBehavior, CAI_GoalEntity *pGoal )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::OnClearGoal( CAI_BehaviorBase *pBehavior, CAI_GoalEntity *pGoal )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_FAT_PLAY_INSPECT_SEQUENCE:
		SetIdealActivity( (Activity) m_nInspectActivity );
		break;

	case TASK_FAT_SIT_ON_TRAIN:
		if ( NameMatches("fatcit_train_2") )
		{
			SetSequenceByName( "d1_t01_TrainRide_Sit_Idle" );
			SetIdealActivity( ACT_DO_NOT_DISTURB );
	...
	case TASK_FAT_HEAL_TOSS:
		...
		SetIdealActivity( (Activity)ACT_FAT_HEAL );
		break;
	
	case TASK_FAT_RPG_AUGER:
		m_bRPGAvoidPlayer = false;
		SetWait( 15.0 ); // maximum time auger before giving up
		break;

	case TASK_FAT_SPEAK_MOURNING:
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
...
// Input  : code - 
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::TaskFail( AI_TaskFailureCode_t code )
{
	// If our heal task has failed, push out the heal time
	if ( IsCurSchedule( SCHED_CITIZEN_HEAL ) )
	{
		m_flPlayerHealTime 	= gpGlobals->curtime + sk_fatcit_heal_ally_delay.GetFloat();
	}
...
//-----------------------------------------------------------------------------
Activity CNPC_FatCitizen::NPC_TranslateActivity( Activity activity )
{
	...
	// !!!HACK - FatCitizens don't have the required animations for shotguns, 
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_FatCitizen::HandleAnimEvent( animevent_t *pEvent )
{
	if ( pEvent->event == AE_FATIZEN_GET_PACKAGE )
	{
		// Give the fatcit a package
		CBaseCombatWeapon *pWeapon = Weapon_Create( "weapon_fatcitpackage" );
		if ( pWeapon )
		{
	...
	else if ( pEvent->event == AE_FATIZEN_HEAL )
	{
		...
		if ( USE_EXPERIMENTAL_MEDIC_CODE() && IsMedic() )
		{
			...
			if ( pTarget )
			{
				m_flPlayerHealTime 	= gpGlobals->curtime + sk_fatcit_heal_toss_player_delay.GetFloat();;
	...
	case NPC_EVENT_LEFTFOOT:
		{
			EmitSound( "NPC_FatCitizen.FootstepLeft", pEvent->eventtime );
		}
		break;

	case NPC_EVENT_RIGHTFOOT:
		{
			EmitSound( "NPC_FatCitizen.FootstepRight", pEvent->eventtime );
		}
		break;
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_FatCitizen::PickupItem( CBaseEntity *pItem )
{
	Assert( pItem != NULL );
	...
	{
		DevMsg("FatCitizen doesn't know how to pick up %s!\n", pItem->GetClassname() );
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::IgnorePlayerPushing( void )
{
...
//-----------------------------------------------------------------------------
const char *CNPC_FatCitizen::SelectRandomExpressionForState( NPC_STATE state )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::SimpleUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	// Under these conditions, fatcits will refuse to go with the player.
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::OnBeginMoveAndShoot()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::OnEndMoveAndShoot()
{
#ifdef EZ
...
void CNPC_FatCitizen::LocateEnemySound()
{
#ifdef EZ
	if ( !GetEnemy() )
		return;
	...
	if( flZDiff < -128 )
	{
		EmitSound( "NPC_FatCitizen.UpThere" );
	}
	else if( flZDiff > 128 )
	{
		EmitSound( "NPC_FatCitizen.DownThere" );
	}
	else
	{
		EmitSound( "NPC_FatCitizen.OverHere" );
	}
#endif
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::IsManhackMeleeCombatant()
{
	CBaseCombatWeapon *pWeapon = GetActiveWeapon();
	CBaseEntity *pEnemy = GetEnemy();
...
//-----------------------------------------------------------------------------
Vector CNPC_FatCitizen::GetActualShootPosition( const Vector &shootOrigin )
{
#ifdef MAPBASE
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::OnChangeActiveWeapon( CBaseCombatWeapon *pOldWeapon, CBaseCombatWeapon *pNewWeapon )
{
	if ( pNewWeapon )
	{
...
bool CNPC_FatCitizen::ShouldLookForBetterWeapon()
{
	if ( BaseClass::ShouldLookForBetterWeapon() )
	{
		...
		{
			// For fatcits in the player squad, you must be unarmed, or standing still (if armed) in order to 
		...
		if( pWeapon )
		{
			...
			else if( FClassnameIs( pWeapon, "weapon_shotgun" ) )
			{
				...
				// If more than one fatcit in the squad has a shotgun, we force
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
#define CITIZEN_SCORCH_FLOOR	75
	...
	{
		if( npc_fatcit_explosive_resist.GetBool() )
		{
			// Blast damage. If this kills a squad member, give the 
			// remaining fatcits a resistance bonus to this inflictor
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::IsCommandable() 
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::IsPlayerAlly( CBasePlayer *pPlayer )											
{ 
	if ( Classify() == CLASS_CITIZEN_PASSIVE && GlobalEntity_GetState("gordon_precriminal") == GLOBAL_ON )
	{
		// Robin: FatCitizens use friendly speech semaphore in trainstation
		return true;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::CanJoinPlayerSquad()
{
	if ( !AI_IsSinglePlayer() )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::WasInPlayerSquad()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::HaveCommandGoal() const			
{	
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::IsCommandMoving()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::ShouldAutoSummon()
{
	...
	float moveTolerance = stupidplayer_squad_autosummon_move_tolerance.GetFloat() * 12;
	...
	{
		m_AutoSummonTimer.Set( stupidplayer_squad_autosummon_time.GetFloat() );
		if ( stupidplayer_squad_autosummon_debug.GetBool() )
	...
	else if ( m_AutoSummonTimer.Expired() )
	{
		...
		if ( gpGlobals->curtime - m_AutoSummonTimer.GetNext() > stupidplayer_squad_autosummon_time.GetFloat() * 2 )
		{
			bSetFollow = true;
			if ( stupidplayer_squad_autosummon_debug.GetBool() )
				DevMsg( "Auto summoning squad: long time (%f)\n", ( gpGlobals->curtime - m_AutoSummonTimer.GetNext() ) + stupidplayer_squad_autosummon_time.GetFloat() );
		}
		...
		if ( distMovedSq > Square(12) )
		{
			...
			{
				float closenessTolerance = stupidplayer_squad_autosummon_player_tolerance.GetFloat() * 12;
		...
		if ( bSetFollow && bTestEnemies )
		{
			...
			{
				CAI_BaseNPC *pNpc = g_AI_Manager.AccessAIs()[i];
				float timeSinceCombatTolerance = stupidplayer_squad_autosummon_time_after_combat.GetFloat();
			...
			if ( !bSetFollow && stupidplayer_squad_autosummon_debug.GetBool() )
...
//-----------------------------------------------------------------------------
// Is this entity something that the fatcit should interact with (return true)
// or something that he should try to get close to (return false)
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::IsValidCommandTarget( CBaseEntity *pTarget )
{
	return false;
}

//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::SpeakCommandResponse( AIConcept_t concept, const char *modifiers )
{
...
//			try to send all 3 selectedfatcits to pick up the same gun.
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::TargetOrder( CBaseEntity *pTarget, CAI_BaseNPC **Allies, int numAllies )
{
	if ( pTarget->IsPlayer() )
	{
...
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::MoveOrder( const Vector &vecDest, CAI_BaseNPC **Allies, int numAllies )
{
	if ( !AI_IsSinglePlayer() )
		return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::OnMoveOrder()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::CommanderUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pActivator == UTIL_GetLocalPlayer() )
	{
		...
		if ( npc_fatcit_auto_stupidplayer_squad_allow_use.GetBool() )
		{
			if ( !ShouldAutosquad() )
				TogglePlayerSquadState();
			else if ( !IsInPlayerSquad() && npc_fatcit_auto_stupidplayer_squad_allow_use.GetBool() )
				AddToPlayerSquad();
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::ShouldSpeakRadio( CBaseEntity *pListener )
{
	if ( !pListener )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::OnMoveToCommandGoalFailed()
{
	// Clear the goal.
	SetCommandGoal( vec3_invalid );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::AddToPlayerSquad()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::RemoveFromPlayerSquad()
{
	Assert( IsInPlayerSquad() );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::TogglePlayerSquadState()
{
	if ( !AI_IsSinglePlayer() )
		return;
...
{
	CNPC_FatCitizen *pFatCitizen;
	bool		  bIsInSquad;
	float		  distSq;
	int			  iSquadIndex;
};

void CNPC_FatCitizen::UpdatePlayerSquad()
{
	if ( !AI_IsSinglePlayer() )
		return;
	...
	if ( pPlayerSquad )
	{
		CUtlVectorFixed<CNPC_FatCitizen *, MAX_PLAYER_SQUAD> squadMembersToRemove;
		AISquadIter_t iter;
		...
		for ( CAI_BaseNPC *pPlayerSquadMember = pPlayerSquad->GetFirstMember(&iter); pPlayerSquadMember; pPlayerSquadMember = pPlayerSquad->GetNextMember(&iter) )
		{
			...
			CNPC_FatCitizen *pFatCitizen = assert_cast<CNPC_FatCitizen *>(pPlayerSquadMember);

			if ( !pFatCitizen->m_bNeverLeavePlayerSquad &&
				 pFatCitizen->m_FollowBehavior.GetFollowTarget() &&
				 !pFatCitizen->m_FollowBehavior.FollowTargetVisible() && 
				 pFatCitizen->m_FollowBehavior.GetNumFailedFollowAttempts() > 0 && 
				 gpGlobals->curtime - pFatCitizen->m_FollowBehavior.GetTimeFailFollowStarted() > 20 &&
				 ( fabsf(( pFatCitizen->m_FollowBehavior.GetFollowTarget()->GetAbsOrigin().z - pFatCitizen->GetAbsOrigin().z )) > 196 ||
				   ( pFatCitizen->m_FollowBehavior.GetFollowTarget()->GetAbsOrigin().AsVector2D() - pFatCitizen->GetAbsOrigin().AsVector2D() ).LengthSqr() > Square(50*12) ) )
			{
				if ( DebuggingCommanderMode() )
				{
					DevMsg( "Player follower is lost (%d, %f, %d)\n", 
						 pFatCitizen->m_FollowBehavior.GetNumFailedFollowAttempts(), 
						 gpGlobals->curtime - pFatCitizen->m_FollowBehavior.GetTimeFailFollowStarted(), 
						 (int)((pFatCitizen->m_FollowBehavior.GetFollowTarget()->GetAbsOrigin().AsVector2D() - pFatCitizen->GetAbsOrigin().AsVector2D() ).Length()) );
				}

				squadMembersToRemove.AddToTail( pFatCitizen );
			}
		}
	...
	if ( pPlayer && ShouldAutosquad() && !(pPlayer->GetFlags() & FL_NOTARGET ) && pPlayer->IsAlive() )
	{
		...
		for ( i = 0; i < g_AI_Manager.NumAIs(); i++ )
		{
			...
			CNPC_FatCitizen *pFatCitizen = assert_cast<CNPC_FatCitizen *>(ppAIs[i]);
			int iNew;

			if ( pFatCitizen->IsInPlayerSquad() )
			{
				iNew = candidates.AddToTail();
				candidates[iNew].pFatCitizen = pFatCitizen;
				candidates[iNew].bIsInSquad = true;
				candidates[iNew].distSq = 0;
				candidates[iNew].iSquadIndex = pFatCitizen->GetSquad()->GetSquadIndex( pFatCitizen );
			}
			else
			{
				float distSq = (vPlayerPos.AsVector2D() - pFatCitizen->GetAbsOrigin().AsVector2D()).LengthSqr(); 
				if ( distSq > JOIN_PLAYER_XY_TOLERANCE_SQ && 
					( pFatCitizen->m_flTimeJoinedPlayerSquad == 0 || gpGlobals->curtime - pFatCitizen->m_flTimeJoinedPlayerSquad > 60.0 ) && 
					( pFatCitizen->m_flTimeLastCloseToPlayer == 0 || gpGlobals->curtime - pFatCitizen->m_flTimeLastCloseToPlayer > 15.0 ) )
					continue;

				if ( !pFatCitizen->CanJoinPlayerSquad() )
					continue;

				bool bShouldAdd = false;

				if ( pFatCitizen->HasCondition( COND_SEE_PLAYER ) )
					bShouldAdd = true;
				else
				{
					bool bPlayerVisible = pFatCitizen->FVisible( pPlayer );
					if ( bPlayerVisible )
					{
						if ( pFatCitizen->HasCondition( COND_HEAR_PLAYER ) )
							bShouldAdd = true;
						else if ( distSq < UNCONDITIONAL_JOIN_PLAYER_XY_TOLERANCE_SQ && fabsf(vPlayerPos.z - pFatCitizen->GetAbsOrigin().z) < UNCONDITIONAL_JOIN_PLAYER_Z_TOLERANCE )
							bShouldAdd = true;
					}
				}
				...
				if ( bShouldAdd )
				{
					...
					AI_Waypoint_t *pPathToPlayer = pFatCitizen->GetPathfinder()->BuildRoute( pFatCitizen->GetAbsOrigin(), vPlayerPos, pPlayer, 5*12, NAV_NONE, true );
		...
		if ( bFoundNewGuy )
		{
			...
			for ( i = 0; i < initialCount; i++ )
				candidates[i].pFatCitizen->AddSpawnFlags( SF_CITIZEN_NOT_COMMANDABLE ); // Prevents double-add
			for ( i = 0; i < initialCount; i++ )
			{
				if ( candidates[i].iSquadIndex == -1 )
				{
					for ( int j = 0; j < g_AI_Manager.NumAIs(); j++ )
					{
						...
						CNPC_FatCitizen *pFatCitizen = assert_cast<CNPC_FatCitizen *>(ppAIs[j]);

						float distSq = (vPlayerPos - pFatCitizen->GetAbsOrigin()).Length2DSqr(); 
						if ( distSq > JOIN_PLAYER_XY_TOLERANCE_SQ )
							continue;

						distSq = (candidates[i].pFatCitizen->GetAbsOrigin() - pFatCitizen->GetAbsOrigin()).Length2DSqr(); 
						...
						if ( !pFatCitizen->FVisible( pPlayer ) )
							continue;
						...
						candidates[iNew].distSq = distSq;
						candidates[iNew].iSquadIndex = -1;
						pFatCitizen->AddSpawnFlags( SF_CITIZEN_NOT_COMMANDABLE ); // Prevents double-add
					}
				}
			}
			for ( i = 0; i < candidates.Count(); i++ )
				candidates[i].pFatCitizen->RemoveSpawnFlags( SF_CITIZEN_NOT_COMMANDABLE );
			...
			if ( candidates.Count() > MAX_PLAYER_SQUAD )
			{
				...
				{
					if ( candidates[i].pFatCitizen->IsInPlayerSquad() )
					{
						candidates[i].pFatCitizen->RemoveFromPlayerSquad();
					}
				}
			}
			...
			if ( candidates.Count() )
			{
				...
				for ( i = 0; i < candidates.Count() && i < MAX_PLAYER_SQUAD; i++ )
				{
					if ( !candidates[i].pFatCitizen->IsInPlayerSquad() )
					{
						candidates[i].pFatCitizen->AddToPlayerSquad();
						nJoined++;

						if ( candidates[i].distSq < closestDistSq )
						{
							pClosest = candidates[i].pFatCitizen;
				...
				if ( pClosest )
				{
					...
					for ( i = 0; i < candidates.Count() && i < MAX_PLAYER_SQUAD; i++ )
					{
						candidates[i].pFatCitizen->SetSpokeConcept( TLK_JOINPLAYER, NULL ); 
					}
				}
			}
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::PlayerSquadCandidateSortFunc( const SquadCandidate_t *pLeft, const SquadCandidate_t *pRight )
{
	// "Bigger" means less approprate 
	CNPC_FatCitizen *pLeftFatCitizen = pLeft->pFatCitizen;
	CNPC_FatCitizen *pRightFatCitizen = pRight->pFatCitizen;

	// Medics are better than anyone
	if ( pLeftFatCitizen->IsMedic() && !pRightFatCitizen->IsMedic() )
		return -1;

	if ( !pLeftFatCitizen->IsMedic() && pRightFatCitizen->IsMedic() )
		return 1;

	CBaseCombatWeapon *pLeftWeapon = pLeftFatCitizen->GetActiveWeapon();
	CBaseCombatWeapon *pRightWeapon = pRightFatCitizen->GetActiveWeapon();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::FixupPlayerSquad()
{
	if ( !AI_IsSinglePlayer() )
		return;
	...
	if ( m_pSquad->NumMembers() > MAX_PLAYER_SQUAD )
	{
		...
		CNPC_FatCitizen *pFirstMemberFatCitizen = dynamic_cast< CNPC_FatCitizen * >( pFirstMember );
		if ( pFirstMemberFatCitizen )
		{
			pFirstMemberFatCitizen->ClearFollowTarget();
		}
		else
		{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::ClearFollowTarget()
{
	m_FollowBehavior.SetFollowTarget( NULL );
	m_FollowBehavior.SetParameters( AIF_SIMPLE );
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::UpdateFollowCommandPoint()
{
	if ( !AI_IsSinglePlayer() )
		return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::IsFollowingCommandPoint()
{
	CBaseEntity *pFollowTarget = m_FollowBehavior.GetFollowTarget();
	if ( pFollowTarget )
...
CAI_BaseNPC *CNPC_FatCitizen::GetSquadCommandRepresentative()
{
	if ( !AI_IsSinglePlayer() )
		return NULL;
	...
	if ( IsInPlayerSquad() )
	{
		...
		if ( gpGlobals->curtime - lastTime > 2.0 || !hCurrent || !hCurrent->IsInPlayerSquad() ) // hCurrent will be NULL after level change
		{
			...
			if ( pPlayer )
			{
				...
				for ( CAI_BaseNPC *pAllyNpc = m_pSquad->GetFirstMember(&iter); pAllyNpc; pAllyNpc = m_pSquad->GetNextMember(&iter) )
				{
					if ( pAllyNpc->IsCommandable() && dynamic_cast<CNPC_FatCitizen *>(pAllyNpc) )
					{
						int i = candidates.AddToTail();
						candidates[i].pMember = (CNPC_FatCitizen *)(pAllyNpc);
		...
		if ( hCurrent != NULL )
		{
			Assert( dynamic_cast<CNPC_FatCitizen *>(hCurrent.Get()) && hCurrent->IsInPlayerSquad() );
			return hCurrent;
		}
	}
	return NULL;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::SetSquad( CAI_Squad *pSquad )
{
	...
	if( IsInPlayerSquad() && !bWasInPlayerSquad )
	{
		...
		if ( npc_fatcit_insignia.GetBool() )
			AddInsignia();
	}
...
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
	// TODO:  As fatcit gets more complex, we will have to only allow
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::FValidateHintType( CAI_Hint *pHint )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::ShouldHealTarget( CBaseEntity *pTarget, bool bActiveUse )
{
			...
			)
	 	{
			if ( pTarget->m_iHealth > 0 )
			{
	 			if ( bActiveUse )
				{
					...
					float timeRecharge = sk_fatcit_heal_player_delay.GetFloat();
					float maximumHealAmount = sk_fatcit_heal_player.GetFloat();
					...
					if ( healAmt < sk_fatcit_heal_player_min_forced.GetFloat() )
						return false;
				...
				if ( bReadyToHeal )
				{
					...
					if ( bTargetIsPlayer )
						requiredHealth = pTarget->GetMaxHealth() - sk_fatcit_heal_player.GetFloat();
					else
						requiredHealth = pTarget->GetMaxHealth() * sk_fatcit_heal_player_min_pct.GetFloat();
	...
	if ( IsAmmoResupplier() && bTargetIsPlayer )
	{
		if ( m_flPlayerGiveAmmoTime <= gpGlobals->curtime )
		{
			...
			if ( iAmmoType == -1 )
			{
				DevMsg("ERROR: FatCitizen attempting to give unknown ammo type (%s)\n", STRING(m_iszAmmoSupply) );
			}
			else
			{
...
#ifdef HL2_EPISODIC
//-----------------------------------------------------------------------------
// Determine if the fatcit is in a position to be throwing medkits
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::ShouldHealTossTarget( CBaseEntity *pTarget, bool bActiveUse )
{
	Disposition_t disposition;
...
void CNPC_FatCitizen::Heal()
{
	...
	m_flTimeNextHealStare = gpGlobals->curtime + sk_fatcit_stare_heal_time.GetFloat();
	...
	if ( IsMedic() )
	{
		...
		if ( pTarget->IsPlayer() )
		{
			...
			timeRecharge 		= sk_fatcit_heal_player_delay.GetFloat();
			maximumHealAmount 	= sk_fatcit_heal_player.GetFloat();
			m_flPlayerHealTime 	= gpGlobals->curtime + timeRecharge;
		}
		else
		{
			timeFullHeal 		= m_flAllyHealTime;
			timeRecharge 		= sk_fatcit_heal_ally_delay.GetFloat();
			maximumHealAmount 	= sk_fatcit_heal_ally.GetFloat();
		...
		if ( healAmt > 0 )
		{
			if ( pTarget->IsPlayer() && npc_fatcit_medic_emit_sound.GetBool() )
			{
	...
	if ( IsAmmoResupplier() )
	{
		...
		if ( pTarget->IsPlayer() )
		{
			...
			m_flPlayerGiveAmmoTime = gpGlobals->curtime + sk_fatcit_giveammo_player_delay.GetFloat();
		}
	}
}
...
//-----------------------------------------------------------------------------
void	CNPC_FatCitizen::TossHealthKit(CBaseCombatCharacter *pThrowAt, const Vector &offset)
{
	Assert( pThrowAt );
	...
	if (npc_fatcit_medic_throw_style.GetInt() == 0)
	{
	...
	{
		Warning("FatCitizen tried to heal but could not spawn item_healthkit!\n");
	}
...
//-----------------------------------------------------------------------------
void	CNPC_FatCitizen::InputForceHealthKitToss( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::ShouldLookForHealthItem()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::InputStartPatrolling( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::InputStopPatrolling( inputdata_t &inputdata )
{
	m_bShouldPatrol = false;
}
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_FatCitizen::OnGivenWeapon( CBaseCombatWeapon *pNewWeapon )
{
	FixupMattWeapon();
}
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_FatCitizen::InputSetCommandable( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::InputSetMedicOn( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::InputSetMedicOff( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::InputSetAmmoResupplierOn( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::InputSetAmmoResupplierOff( inputdata_t &inputdata )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_FatCitizen::InputSpeakIdleResponse( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_FatCitizen::DeathSound( const CTakeDamageInfo &info )
{
	// Sentences don't play on dead NPCs
	SentenceStop();

	EmitSound( "NPC_FatCitizen.Die" );
#ifdef EZ
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_FatCitizen::FearSound( void )
{
#ifdef EZ
...
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::UseSemaphore( void )
{
...
AI_BEGIN_CUSTOM_NPC( npc_fatcit, CNPC_FatCitizen )

	DECLARE_TASK( TASK_FAT_HEAL )
	DECLARE_TASK( TASK_FAT_RPG_AUGER )
	DECLARE_TASK( TASK_FAT_PLAY_INSPECT_SEQUENCE )
	DECLARE_TASK( TASK_FAT_SIT_ON_TRAIN )
	DECLARE_TASK( TASK_FAT_LEAVE_TRAIN )
	DECLARE_TASK( TASK_FAT_SPEAK_MOURNING )
#if HL2_EPISODIC
	DECLARE_TASK( TASK_FAT_HEAL_TOSS )
#endif

	DECLARE_ACTIVITY( ACT_FAT_HANDSUP )
	DECLARE_ACTIVITY( ACT_FAT_BLINDED )
	DECLARE_ACTIVITY( ACT_FAT_SHOWARMBAND )
	DECLARE_ACTIVITY( ACT_FAT_HEAL )
	DECLARE_ACTIVITY( ACT_FAT_STARTLED )

	DECLARE_CONDITION( COND_FAT_PLAYERHEALREQUEST )
	DECLARE_CONDITION( COND_FAT_COMMANDHEAL )
	DECLARE_CONDITION( COND_FAT_START_INSPECTION )
#ifdef EZ
	DECLARE_CONDITION(COND_FAT_WILLPOWER_LOW)
	DECLARE_CONDITION(COND_FAT_WILLPOWER_HIGH)
	...
	//Events
	DECLARE_ANIMEVENT( AE_FATIZEN_GET_PACKAGE )
	DECLARE_ANIMEVENT( AE_FATIZEN_HEAL )
...
//		"		TASK_PLAY_SEQUENCE_FACE_TARGET		ACTIVITY:ACT_DISARM"
	...
	// this is for the episodic behavior where the fatcit hurls the medkit
	//=========================================================
	DEFINE_SCHEDULE
	(
		SCHED_CITIZEN_HEAL_TOSS,
...
//	"		TASK_PLAY_SEQUENCE_FACE_TARGET		ACTIVITY:ACT_DISARM"
	...
	(
		...
		"	Tasks"
		"		TASK_STOP_MOVING				0"
		"		TASK_FAT_PLAY_INSPECT_SEQUENCE	0"	// Play the sequence the scanner requires
		"		TASK_WAIT						2"
		""
		"	Interrupts"
		"		"
	...
	(
		...
		"	Tasks"
		"		TASK_FAT_SIT_ON_TRAIN		0"
		"		TASK_WAIT_RANDOM			1"
		"		TASK_FAT_LEAVE_TRAIN		0"
		""
		"	Interrupts"
	)
#ifdef EZ
...
//
//==================================================================================================================
CHandle<CFatCitizenResponseSystem>	g_pFatCitizenResponseSystem = NULL;

CFatCitizenResponseSystem	*GetFatCitizenResponse()
{
	return g_pFatCitizenResponseSystem;
}

char *FatCitizenResponseConcepts[MAX_CITIZEN_RESPONSES] = 
{
...
LINK_ENTITY_TO_CLASS( ai_fatcit_response_system, CFatCitizenResponseSystem );

BEGIN_DATADESC( CFatCitizenResponseSystem )
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CFatCitizenResponseSystem::Spawn()
{
	if ( g_pFatCitizenResponseSystem )
	{
		Warning("Multiple fatcit response systems in level.\n");
		UTIL_Remove( this );
		return;
	}
	g_pFatCitizenResponseSystem = this;

	// Invisible, non solid.
	AddSolidFlags( FSOLID_NOT_SOLID );
	AddEffects( EF_NODRAW );
	SetThink( &CFatCitizenResponseSystem::ResponseThink );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CFatCitizenResponseSystem::OnRestore()
{
	BaseClass::OnRestore();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CFatCitizenResponseSystem::AddResponseTrigger( fatcitresponses_t	iTrigger )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CFatCitizenResponseSystem::InputResponseVitalNPC( inputdata_t &inputdata )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CFatCitizenResponseSystem::ResponseThink()
{
	...
	if ( AI_IsSinglePlayer() )
	{
		for ( int i = 0; i < MAX_CITIZEN_RESPONSES; i++ )
		{
			if ( m_flResponseAddedTime[i] )
			{
				...
				{
					// Try and find the nearest fatcit to the player
					float flNearestDist = (CITIZEN_RESPONSE_DISTANCE * CITIZEN_RESPONSE_DISTANCE);
					CBaseEntity *pNearestFatCitizen = NULL;
					CBaseEntity *pFatCitizen = NULL;
					CBasePlayer *pPlayer = UTIL_GetLocalPlayer();
					while ( (pFatCitizen = gEntList.FindEntityByClassname( pFatCitizen, "npc_fatcit" ) ) != NULL)
					{
						float flDistToPlayer = (pPlayer->WorldSpaceCenter() - pFatCitizen->WorldSpaceCenter()).LengthSqr();
						if ( flDistToPlayer < flNearestDist )
						{
							flNearestDist = flDistToPlayer;
							pNearestFatCitizen = pFatCitizen;
						}
					}

					// Found one?
					if ( pNearestFatCitizen && ((CNPC_FatCitizen*)pNearestFatCitizen)->RespondedTo( FatCitizenResponseConcepts[i], false, false ) )
					{
						m_flResponseAddedTime[i] = 0;
...
void CNPC_FatCitizen::AddInsignia()
{
...
void CNPC_FatCitizen::RemoveInsignia()
{
	...
	while( pEntity )
	{
		...
		{
			// Is this my insignia?
			CSquadFatInsignia *pInsignia = dynamic_cast<CSquadFatInsignia *>(pEntity);
...
//-----------------------------------------------------------------------------
LINK_ENTITY_TO_CLASS( squadinsignia, CSquadFatInsignia );

void CSquadFatInsignia::Spawn()
{
...
//-----------------------------------------------------------------------------
int CNPC_FatCitizen::DrawDebugTextOverlays( void ) 
{
	...
	if (m_debugOverlays & OVERLAY_TEXT_BIT) 
	{
		...
		Q_snprintf(tempstr,sizeof(tempstr),"Expression type: %s", szExpressionTypesFat[m_ExpressionType]);
...
#ifdef EZ
//------------------------------------------------------------------------------
// Added by 1upD. FatCitizen weapon proficiency should be configurable
//------------------------------------------------------------------------------
WeaponProficiency_t CNPC_FatCitizen::CalcWeaponProficiency(CBaseCombatWeapon *pWeapon)
{
	int proficiency;

	if (FClassnameIs(pWeapon, "weapon_ar2"))
	{
		proficiency = sk_fatcit_ar2_proficiency.GetInt();
	}
	else {
		proficiency = sk_fatcit_default_proficiency.GetInt();
	}
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_FatCitizen::IsJumpLegal(const Vector &startPos, const Vector &apex, const Vector &endPos) const
{
	if (m_Type == CT_LONGFALL) 
	{
