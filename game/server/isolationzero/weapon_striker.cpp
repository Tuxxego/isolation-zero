...
//
// Purpose:		Super powerful striker.
//
// $NoKeywords: $
//=============================================================================//
...
ConVar	striker_use_new_accuracy( "striker_use_new_accuracy", "1" );

//-----------------------------------------------------------------------------
// CWeaponStriker
//-----------------------------------------------------------------------------

class CWeaponStriker : public CHLMachineGun
{
	DECLARE_DATADESC();

public:
	DECLARE_CLASS(CWeaponStriker, CHLMachineGun);
	...
	{		
		...
		if ( striker_use_new_accuracy.GetBool() )
		{
...
#ifdef MAPBASE
	// Strikers are their own backup activities
...
#ifdef MAPBASE
// Allows Weapon_BackupActivity() to access the striker's activity table.
acttable_t *GetStrikerActtable()
{
	return CWeaponStriker::m_acttable;
}

int GetStrikerActtableCount()
{
	return ARRAYSIZE(CWeaponStriker::m_acttable);
}
#endif
...
void CWeaponStriker::PrimaryAttack( void )
{
	...
	if( pOwner )
	{
		// Each time the player fires the striker, reset the view punch. This prevents
...
ConVar sv_pulse_striker_style( "sv_pulse_striker_style", "0", FCVAR_NONE, "Style of pulse striker altfire: 0) Default 1) Charge 2) Lance" );
ConVar sv_pulse_lance_style( "sv_pulse_lance_style", "0", FCVAR_NONE, "Style of pulse lance: 0) Vortigaunt 1) Stalker" );
ConVar sv_pulse_striker_charge_per_extra_shot( "sv_pulse_striker_charge_per_extra_shot", "5", FCVAR_NONE, "How many ammo charges equals one bullet" );
ConVar sv_pulse_striker_slide_return_charge( "sv_pulse_striker_slide_return_charge", "10", FCVAR_NONE, "How much charge to restore when racking the slide on the pulse striker" );
ConVar sv_pulse_striker_no_charge_hold( "sv_pulse_striker_no_charge_hold", "0", FCVAR_NONE, "If 1, fully charged shots will fire automatically" );
ConVar sv_pulse_striker_max_charge( "sv_pulse_striker_max_charge", "40", FCVAR_NONE, "Maximum ammo to charge in one shot. For a 50 charge pulse striker, it should be 40" );

//-----------------------------------------------------------------------------
// CWeaponPulseStriker, the famous melee replacement taken to a separate class.
//-----------------------------------------------------------------------------
class CWeaponPulseStriker : public CWeaponStriker
{
	DECLARE_DATADESC();
public:
	DECLARE_CLASS(CWeaponPulseStriker, CWeaponStriker);
	DECLARE_SERVERCLASS();
	...
	virtual bool Reload( void ) { return false; } // The pulse striker does not reload

	virtual int GetMaxClip2( void ) const { int iBase = BaseClass::GetMaxClip2(); return iBase > 0 ? iBase : sv_pulse_striker_max_charge.GetInt(); }
...
CWeaponPulseStriker::CWeaponPulseStriker()
{
	m_iClip2 = 0;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::Activate( void )
{
	BaseClass::Activate();
...
// Purpose: Precache
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::Precache( void )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::UpdateOnRemove( void )
{
...
bool CWeaponPulseStriker::IsChargePressed( int chargeButton, CBasePlayer * pOwner )
{
	// If "no charge hold" is set, and we're at the maximum charge, treat it as though the player let go of the attack button
	if (sv_pulse_striker_no_charge_hold.GetBool() && m_iClip2 >= GetMaxClip2())
		return false;
	...
	if (sv_pulse_striker_no_charge_hold.GetBool() && m_iClip2 > 0 && m_iClip1 <= 10)
		return false;
...
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::ItemPostFrame( void )
{
	// Charge up and fire attack
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );

	bool chargePressed = false;
	switch ( sv_pulse_striker_style.GetInt() )
	{
	case PRIMARY_CHARGE:
...
//-----------------------------------------------------------------------------
// Purpose: The pulse striker has a longer dryfire refire time than the 9mm
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::DryFire( void )
{
	WeaponSound( EMPTY );
	SendWeaponAnim( ACT_VM_DRYFIRE );
...
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::Operator_HandleAnimEvent( animevent_t * pEvent, CBaseCombatCharacter * pOperator )
{
	switch (pEvent->event)
	{
	case AE_WPN_INCREMENTAMMO:
	case AE_SLIDERETURN:
		m_iClip1 = MAX( m_iClip1, sv_pulse_striker_slide_return_charge.GetInt() );
...
//-----------------------------------------------------------------------------
bool CWeaponPulseStriker::Deploy( void )
{
	RechargeAmmo();
...
//-----------------------------------------------------------------------------
bool CWeaponPulseStriker::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	if (m_iClip2 > 0)
	{
...
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::RechargeAmmo( void )
{
	...
	// 1upD - how long as the striker been charging?
	float flChargeInterval = gpGlobals->curtime - m_flLastChargeTime;
	m_flLastChargeTime = gpGlobals->curtime;

	// This code is inherited from the airboat. I've changed it to work for the striker.
	int nMaxAmmo = 50;
...
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::MakeTracer( const Vector &vecTracerSrc, const trace_t &tr, int iTracerType )
{
	float flTracerDist;
	Vector vecDir;
	Vector vecEndPos;
...
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::PrimaryAttack( void )
{
	...
	else
	{
		...
		int iExtraChargeBullets = m_iClip2 / sv_pulse_striker_charge_per_extra_shot.GetInt();

		// The pulse striker fires one extra round per ten ammo charged
		...
		while (m_flNextPrimaryAttack <= gpGlobals->curtime)
		{
			...
			// Pulse striker fires two bullets
			iBulletsToFire += 2;
		...
		// Add to the accuracy penalty just like standard striker
		m_flAccuracyPenalty += PISTOL_ACCURACY_SHOT_PENALTY_TIME;
...
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::ChargeAttack( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::AddViewKick( void )
{
	CBasePlayer *pPlayer  = ToBasePlayer( GetOwner() );
...
//			nDamageType - 
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::DoImpactEffect( trace_t &tr, int nDamageType )
{
	CEffectData data;
...
//-----------------------------------------------------------------------------
// Purpose: Particle muzzle flash for pulse striker
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::DoMuzzleFlash( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	...
	data.m_flScale = 0.25f; // The striker is much smaller than a Hunter-Chopper ( ._.)
...
// and sound of the pulse striker alt-fire, but this is sufficent for alpha
...
//-----------------------------------------------------------------------------
// Purpose: Create effects for pulse striker alt-fire
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::StartChargeEffects()
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::SetChargeEffectBrightness( float alpha )
{
...
//-----------------------------------------------------------------------------
// Purpose: Remove pulse striker alt-fire effects
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::KillChargeEffects()
{
	if (m_hChargeSprite != NULL)
	{
...
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::SecondaryAttack()
{
	CBasePlayer *pPlayer  = ToBasePlayer( GetOwner() );
...
//-----------------------------------------------------------------------------
void CWeaponPulseStriker::CreateBeamBlast( const Vector &vecOrigin )
{
...
IMPLEMENT_SERVERCLASS_ST(CWeaponPulseStriker, DT_WeaponPulseStriker)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS( weapon_pulsestriker, CWeaponPulseStriker );
PRECACHE_WEAPON_REGISTER( weapon_pulsestriker );

BEGIN_DATADESC( CWeaponPulseStriker )
