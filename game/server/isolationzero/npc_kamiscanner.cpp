...
//-----------------------------------------------------------------------------
// Parameters for how the kamiscanner relates to citizens.
//-----------------------------------------------------------------------------
...
ConVar	sk_kamiscanner_health( "sk_kamiscanner_health","0");
ConVar	g_debug_ckamiscanner( "g_debug_ckamiscanner", "0" );
...
BEGIN_DATADESC( CNPC_KamiScanner )
...
//	DEFINE_FIELD( m_hSpotlightTarget,		FIELD_EHANDLE ),
	...
	DEFINE_FIELD( m_bIsAssScanner,			FIELD_BOOLEAN ),
...
LINK_ENTITY_TO_CLASS(npc_kamiscanner, CNPC_KamiScanner); // sanner


//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
CNPC_KamiScanner::CNPC_KamiScanner()
{
#ifdef _DEBUG
	m_vInspectPos.Init();
	...
	{
		// Streetwar kamiscanners are claw kamiscanners
		m_bIsAssScanner = true;
	}
	else
	{
		m_bIsAssScanner = false;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::Spawn(void)
{
	// Check for user error
	if (m_flSpotlightMaxLength <= 0)
	{
		DevMsg("CNPC_KamiScanner::Spawn: Invalid spotlight length <= 0, setting to 500\n");
		m_flSpotlightMaxLength = 500;
	}
	
	if (m_flSpotlightGoalWidth <= 0)
	{
		DevMsg("CNPC_KamiScanner::Spawn: Invalid spotlight width <= 0, setting to 100\n");
		m_flSpotlightGoalWidth = 100;
	}

	if (m_flSpotlightGoalWidth > MAX_BEAM_WIDTH )
	{
		DevMsg("CNPC_KamiScanner::Spawn: Invalid spotlight width %.1f (max %.1f).\n", m_flSpotlightGoalWidth, MAX_BEAM_WIDTH );
		m_flSpotlightGoalWidth = MAX_BEAM_WIDTH; 
	}
	...
	if( m_bIsAssScanner )
	{
		SetModel( DefaultOrCustomModel( "models/shield_kamiscanner.mdl" ) );
	}
	else
	{
		SetModel( DefaultOrCustomModel( "models/kamiscanner.mdl" ) );
	}

	m_iHealth				= sk_kamiscanner_health.GetFloat();
...
//
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::Activate()
{
	BaseClass::Activate();
...
//-----------------------------------------------------------------------------
bool CNPC_KamiScanner::KeyValue( const char *szKeyName, const char *szValue )
{
...
//------------------------------------------------------------------------------
int CNPC_KamiScanner::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
// Purpose:
//------------------------------------------------------------------------------
void CNPC_KamiScanner::Gib( void )
{
	...
	{
		CGib::SpawnSpecificGibs( this, 1, 500, 250, "models/gibs/kamiscanner_gib01.mdl" );
		CGib::SpawnSpecificGibs( this, 1, 500, 250, "models/gibs/kamiscanner_gib02.mdl" );
		CGib::SpawnSpecificGibs( this, 1, 500, 250, "models/gibs/kamiscanner_gib04.mdl" );
		CGib::SpawnSpecificGibs( this, 1, 500, 250, "models/gibs/kamiscanner_gib05.mdl" );
	}
...
//			bitsDamageType - 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( !m_bIsAssScanner && GetEnemy() != NULL && (info.GetDamageType() & DMG_DISSOLVE) == false )
	{
		...
		{	
			...
			// This is especially bad if someone machineguns the divebombing kamiscanner. 
			AttackDivebomb();
			return;
		}
	}
...
//-----------------------------------------------------------------------------
bool CNPC_KamiScanner::FValidateHintType(CAI_Hint *pHint)
{
...
// Input  : Type - 
//-----------------------------------------------------------------------------
int CNPC_KamiScanner::TranslateSchedule( int scheduleType ) 
{
	switch ( scheduleType )
	{
...
// Output : int
//-----------------------------------------------------------------------------
Activity CNPC_KamiScanner::NPC_TranslateActivity( Activity eNewActivity )
{
	if( !m_bIsAssScanner )
	{
	...
	// The claw kamiscanner came along a little late and doesn't have the activities
	// of the city kamiscanner. So Just pick between these three
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::HandleAnimEvent( animevent_t *pEvent )
{
	if( pEvent->event == AE_KAMISCANNER_CLOSED )
	{
		m_bIsOpen = false;
...
// Purpose:
//-----------------------------------------------------------------------------
char *CNPC_KamiScanner::GetEngineSound( void )
{
	if( m_bIsAssScanner )
...
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::NPCThink(void)
{
	if (!IsAlive())
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::Precache(void)
{
	// Model
	if( m_bIsAssScanner )
	{
		PrecacheModel( DefaultOrCustomModel( "models/shield_kamiscanner.mdl" ) );
	...
	{
		PrecacheModel( DefaultOrCustomModel( "models/combine_kamiscanner.mdl" ) );

		PrecacheModel("models/gibs/kamiscanner_gib01.mdl" );
		PrecacheModel("models/gibs/kamiscanner_gib02.mdl" );	
		PrecacheModel("models/gibs/kamiscanner_gib02.mdl" );
		PrecacheModel("models/gibs/kamiscanner_gib04.mdl" );
		PrecacheModel("models/gibs/kamiscanner_gib05.mdl" );
...
//------------------------------------------------------------------------------
void CNPC_KamiScanner::RequestInspectSupport(void)
{
	if (m_pSquad)
	{
		AISquadIter_t iter;
...
// Purpose:
//------------------------------------------------------------------------------
bool CNPC_KamiScanner::IsValidInspectTarget(CBaseEntity *pEntity)
{
...
// Purpose:
//------------------------------------------------------------------------------
CBaseEntity* CNPC_KamiScanner::BestInspectTarget(void)
{
	if ( !m_bShouldInspect )
		return NULL;
...
//------------------------------------------------------------------------------
void CNPC_KamiScanner::SetInspectTargetToEnt(CBaseEntity *pEntity, float fInspectDuration)
{
	ClearInspectTarget();
	SetTarget(pEntity);
...
//------------------------------------------------------------------------------
void CNPC_KamiScanner::SetInspectTargetToHint(CAI_Hint *pHint, float fInspectDuration)
{
	ClearInspectTarget();
	...
	if ( g_debug_ckamiscanner.GetBool() )
	{
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_KamiScanner::SetInspectTargetToPos(const Vector &vInspectPos, float fInspectDuration)
{
	ClearInspectTarget();
...
//------------------------------------------------------------------------------
void CNPC_KamiScanner::ClearInspectTarget(void)
{
...
//------------------------------------------------------------------------------
bool CNPC_KamiScanner::HaveInspectTarget( void )
{
	if ( GetTarget() != NULL )
		return true;
...
// Purpose: 
//------------------------------------------------------------------------------
Vector CNPC_KamiScanner::InspectTargetPosition(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::InputShouldInspect( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::DeployMine()
{
	CBaseEntity *child;
...
// Purpose: 
//-----------------------------------------------------------------------------
float CNPC_KamiScanner::GetMaxSpeed()
{
#ifdef MAPBASE
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::InputDeployMine(inputdata_t &inputdata)
{
	DeployMine();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::InputEquipMine(inputdata_t &inputdata)
{
	CBaseEntity *child;
	// iterate through all children
	for ( child = FirstMoveChild(); child != NULL; child = child->NextMovePeer() )
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::InputDisablePhotos( inputdata_t &inputdata )
{
	CapabilitiesRemove(bits_CAP_INNATE_MELEE_ATTACK1);
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::InputEnablePhotos( inputdata_t &inputdata )
{
	CapabilitiesAdd(bits_CAP_INNATE_MELEE_ATTACK1);
}
#endif
...
//-----------------------------------------------------------------------------
// Purpose: Tells the kamiscanner to go photograph an entity.
// Input  : String name or classname of the entity to inspect.
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::InputInspectTargetPhoto(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
// Purpose: Tells the kamiscanner to go spotlight an entity.
// Input  : String name or classname of the entity to inspect.
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::InputInspectTargetSpotlight(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
// Purpose: Tells the kamiscanner to go photo or spotlight an entity.
// Input  : String name or classname of the entity to inspect.
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::InspectTarget( inputdata_t &inputdata, ScannerFlyMode_t eFlyMode )
{
...
//-----------------------------------------------------------------------------
bool CNPC_KamiScanner::MovingToInspectTarget( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::GatherConditions( void )
{
...
// Purpose:
//------------------------------------------------------------------------------
void CNPC_KamiScanner::PrescheduleThink(void)
{
...
//-----------------------------------------------------------------------------
Disposition_t CNPC_KamiScanner::IRelationType(CBaseEntity *pTarget)
{
...
// Input  : *pTask - 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
//			bits.
//-----------------------------------------------------------------------------
int CNPC_KamiScanner::SelectSchedule(void)
{
	...
	if ( HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE) )
	{
		...
		{
			if ( m_iHealth < ( 3 * sk_kamiscanner_health.GetFloat() / 4 ))
...
// Purpose:
//------------------------------------------------------------------------------
void CNPC_KamiScanner::SpotlightDestroy(void)
{
	if ( m_hSpotlight )
	{
...
// Purpose:
//------------------------------------------------------------------------------
void CNPC_KamiScanner::SpotlightCreate(void)
{
	...
	// YWB:  Because the kamiscanner only moves the target during think, make sure we interpolate over 0.1 sec instead of every tick!!!
...
// Purpose:
//------------------------------------------------------------------------------
Vector CNPC_KamiScanner::SpotlightTargetPos(void)
{
	// ----------------------------------------------
	//  If I have an enemy 
	// ----------------------------------------------
...
// Purpose:
//------------------------------------------------------------------------------
Vector CNPC_KamiScanner::SpotlightCurrentPos(void)
{
	Vector vTargetDir		= SpotlightTargetPos() - GetLocalOrigin();
	VectorNormalize(vTargetDir);
...
//------------------------------------------------------------------------------
void CNPC_KamiScanner::SpotlightUpdate(void)
{
	//FIXME: JDW - E3 Hack
	if ( m_bNoLight )
	{
...
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::UpdateOnRemove( void )
{
	SpotlightDestroy();
...
// Purpose:
//------------------------------------------------------------------------------
void CNPC_KamiScanner::TakePhoto(void)
{
#ifdef MAPBASE
...
// Purpose:
//------------------------------------------------------------------------------
void CNPC_KamiScanner::AttackPreFlash(void)
{
	ScannerEmitSound( "TakePhoto" );
...
// Purpose:
//------------------------------------------------------------------------------
void CNPC_KamiScanner::AttackFlash(void)
{
...
// Input  : *pTarget - 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::BlindFlashTarget( CBaseEntity *pTarget )
{
...
// Purpose:
//------------------------------------------------------------------------------
void CNPC_KamiScanner::AttackFlashBlind(void)
{
	if( GetEnemy() )
	{
...
// Purpose:
//------------------------------------------------------------------------------
void CNPC_KamiScanner::AttackDivebomb( void )
{
	if (m_hSpotlight)
	{
		SpotlightDestroy();
...
// Input  : pTask - 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::StartTask( const Task_t *pTask )
{
	switch (pTask->iTask)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
char *CNPC_KamiScanner::GetScannerSoundPrefix( void )
{
	if( m_bIsAssScanner )
		return "NPC_SScanner";
...
// Purpose:
//------------------------------------------------------------------------------
float CNPC_KamiScanner::MinGroundDist( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::AdjustScannerVelocity( void )
{
	if ( m_bIsAssScanner )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_KamiScanner::OverrideMove( float flInterval )
{
	// ----------------------------------------------
	//	If dive bombing
	// ----------------------------------------------
...
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::MoveToTarget( float flInterval, const Vector &vecMoveTarget )
{
...
// Input  : flInterval - 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::MoveToSpotlight( float flInterval )
{
	if ( flInterval <= 0 )
		return;
...
// Output : float
//-----------------------------------------------------------------------------
float CNPC_KamiScanner::GetGoalDistance( void )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::MoveToPhotograph(float flInterval)
{
	if ( HaveInspectTarget() == false )
		return;
...
//-----------------------------------------------------------------------------
bool CNPC_KamiScanner::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* pSourceEnt)
{
	...
	if (interactionType ==	g_interactionScannerSupportEntity)
	{
		...
		if (GetEnemy() == NULL && !HaveInspectTarget())
		{
			...
			{
				float fInspectTime = (((CNPC_KamiScanner*)pSourceEnt)->m_fInspectEndTime - gpGlobals->curtime);
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::InputDisableSpotlight( inputdata_t &inputdata )
{
	m_bNoLight = true;
}
...
// Output : float
//-----------------------------------------------------------------------------
float CNPC_KamiScanner::GetHeadTurnRate( void ) 
{ 
	if ( GetEnemy() )
		return 800.0f;
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::InputSetFollowTarget( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_KamiScanner::InputClearFollowTarget( inputdata_t &inputdata )
{
...
AI_BEGIN_CUSTOM_NPC( npc_ckamiscanner, CNPC_KamiScanner )
	...
	DECLARE_ANIMEVENT( AE_KAMISCANNER_CLOSED )
...
// Claw Scanner
//
// Scanner that always spawns as a claw kamiscanner
//-----------------------------------------------------------------------------
	
class CNPC_AssScanner : public CNPC_KamiScanner
{
DECLARE_CLASS( CNPC_AssScanner, CNPC_KamiScanner );
...
BEGIN_DATADESC( CNPC_AssScanner )
END_DATADESC()


LINK_ENTITY_TO_CLASS(npc_clawkamiscanner, CNPC_AssScanner);

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
CNPC_AssScanner::CNPC_AssScanner()
{
